                                 ...
                                 margin = 50)
{
    ...
    const legends_items = legendContainer.selectAll("legends")
        ...
        .on("click", (event, category) => {
            ...
            const isActive = Fig.selectAll(`circle.${category}`).classed("legendSelect");
    
            // Reset all points to visible
            Fig.selectAll("circle").classed("legendSelect", false);
    
            // If not active, hide other categories and show the selected one
            if (!isActive) {
                Fig.selectAll("circle").classed("legendSelect", false); // Hide all points
                Fig.selectAll(`circle.${category}`).classed("legendSelect", true); // Show selected category
            }
        });
